...
const Navbar = () => {
  ...
  const navStyles = (path) =>
    `relative px-4 py-2 transition-all duration-300 text-[11px] uppercase tracking-[0.2em] font-bold rounded-full ${
      pathname === path ? "bg-[#2D336B] text-white shadow-md" : "text-slate-500 hover:text-[#2D336B] hover:bg-slate-50"
    }`;

  // Specialized style for the "Add Watch" button to make it pop
  const addWatchStyle = (isActive) =>
    `px-5 py-2 text-[11px] uppercase tracking-[0.2em] font-black bg-[#E07A7A] text-white rounded-full hover:bg-[#c96565] transition-all shadow-lg shadow-red-100 flex items-center gap-2 ${
      isActive ? "ring-2 ring-offset-2 ring-[#E07A7A]" : ""
    }`;
  ...
  const li = (
    <>
      ...
      <Link href="/collections" className={navStyles("/collections")}>
        Collections
      </Link>

      {/* Featured "Add Watch" Link */}
      <Link href="/add-watch" className={addWatchStyle(pathname === "/add-watch")}>
          ...
          <div className="flex items-center gap-4">
            {user ? (
                ...
                {dropdown && (
                  <div className="absolute right-0 mt-4 w-64 bg-white border border-slate-100 shadow-2xl rounded-[1.5rem] overflow-hidden animate-in fade-in zoom-in duration-200">
                    ...
                    <div className="p-4 pt-0 flex flex-col gap-2">
                      <Link href="/profile" className="w-full py-2 text-center text-[10px] font-bold uppercase tracking-widest text-slate-500 hover:text-[#2D336B] transition-colors">
                        Settings
                      </Link>
                      <button
              ...
              <div className="flex items-center gap-3">
                <Link href="/auth/login" className="hidden sm:block text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 hover:text-[#2D336B] transition-colors">
                  Login
                </Link>
                <Link
                  href="/auth/register"
